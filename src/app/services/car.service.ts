...
export class CarService {
  ...
  getAllCars() {
    return this.store.select<Car[]>(state => state.app.cars.sort((a, b) => a.name.localeCompare(b.name)));
  }

  getCarById(id: number) {
    return this.store.select<Car>(state => state.app.cars.filter(c => c.id === id)[0]);
  }
